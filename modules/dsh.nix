...
let
  ...
  # 局域网设置读写补丁（见 packages/dsh.nix 的 allowLanSettings）已废弃：
  # dsh ≥ 0.1.5 的 $host 客户端服务不再暴露连接状态（无 $host.state），
  # 旧补丁用 state.getSnapshot() 会抛 "Cannot read properties of undefined"
  # 使整个前端白屏；服务端 Host fence（trustedHosts）仍是唯一边界，非
  # loopback 页面的 settings 保持上游行为（memory，只读）。
  dshPackage = cfg.package;
