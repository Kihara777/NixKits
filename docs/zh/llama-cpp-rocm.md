...
在 Strix Halo（Ryzen AI Max，128 GiB 统一内存）上部署 DeepSeek-V4-Flash-Vision 的实测记录。覆盖 **IQ1_S（1.5625 bpw）** 与 **IQ3_S（3.4375 bpw）** 两种量化。
...
| 量化 | 1.5625 bpw（82.4 GB） | **3.4375 bpw（114.4 GB）** |
| 参数 | 总 284 B（MoE） | 总 284 B（MoE） |
| 硬件 | Strix Halo / Radeon 8060S（`gfx1151`，统一内存） | 同左 |
| 后端 | ROCm，llama.cpp 0.4.0 | 同左 |

### 量化开销不是固定值

装载 IQ3_S 前的预估是"固定开销约 3.7 GiB"，**实测偏差近一个量级**：

| 量化 | 权重文件 | 装载后 GPUActive | 超出权重的开销 |
|------|---------|-----------------|--------------|
| IQ1_S | 82.4 GiB | ≈ 88.9 GiB | **≈ 6.5 GiB** |
| IQ3_S | 114.4 GiB | **112.8–112.9 GiB** | **≈ 13.3 GiB**† |

> † IQ3_S 的权重经 4 分片（shard）拆分后实际落盘略小于标称值，故此处开销按
> "权重分片实际占用"与 GPUActive 之差计算。

**结论：不要用固定常数外推开销。** 开销随量化精度、分片方式、
KV cache 量化与 `fit` 的自适应结果共同变化。**换量化后必须重新实测 GPUActive**，
不能沿用上一档的数字。

**IQ3_S 的余量**：GPUActive 112.8–112.94 GiB 时系统仅剩约 5.8–6 GiB，
GTT 上限 124.9 GiB。实测通过 15013 token 与 40012 token 的 prefill 未 OOM，
仅 179 页被换出。**但余量已很紧**，因此：
...
| **提高量化精度** | IQ1_S 1.56 bpw → IQ3_S 3.44 bpw，**生成速度不变**（12.8 → 12.9 t/s） |
| **提高 GPU 频率** | performance 档相比 balanced 多耗 54% 功耗，生成仅 +2.4% |

### 生成速度：瓶颈是依赖延迟

**提高权重精度不提升生成速度**：权重从 1.56 bpw 升至 3.44 bpw（体积 +39%），
生成速度 12.8 → 12.9 t/s，**在噪声范围内**。若瓶颈在显存带宽，精度提升本应显著降速。

**并发不提升聚合吞吐**：3 个并发请求的聚合吞吐仍为 12.5 t/s，
与单请求相同 —— 说明瓶颈不在吞吐能力。

**频率几乎不影响生成**：performance 档 sclk 2778 MHz、功耗 70.2 W，
生成 12.9 t/s；balanced 档 2436 MHz、45.6 W，生成 12.6 t/s。
**多耗 54% 功耗只换 2.4% 速度。**

三条证据共同指向：**生成受限于逐 token 的依赖延迟**，而非带宽、算力或功耗。
这也是低量化对生成影响甚微、对 prefill 影响显著的原因。

### 功耗档位实测

同一提示词、同一会话、400 token 生成：
...
按"每瓦吞吐"衡量，quiet 是三者中效率最高的。

### 关键结论

**低比特量化的双面性**：低比特对**生成**有利（权重带宽小），但对 **prefill** 不利 —— 每层需反量化权重，而 prefill 是计算密集阶段，反量化算力占比随位宽降低而升高。实测 prefill 仅 142–169 t/s，而生成 12.8 t/s，比值约 11–13×（GPU 上健康值通常 15–30×）。
...
>
> 注意：真实的显存指标是 `/proc/meminfo` 的 **`GPUActive`**，
> **不是** `mem_info_gtt_used`（后者在统一内存设备上不反映实际占用）。
