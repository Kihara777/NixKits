...
本插件支持两种安装方式。**NixOS 用户推荐声明式**（版本由 Nix 锁定、随系统代际更新、可复现）；其他平台的 DSH 用户可用 `dsh plugin add`。**两者择一即可**——它们都会注册同一个 `api-balance` entry id。

### 方式 A：声明式（NixOS 模块，推荐）
...
> **版本可复现性**：方式 B 经 git 解析，**不受 `flake.lock` 锁定**；需要可复现环境请用方式 A。
