...
每次更新流程**全部完成后**（含文档同步与维护日志推送）执行。这一步不看软件，
看的是**决定软件被如何更新的那套规范本身**——技能、`AGENTS.md`、`SECURITY.md`
及相关 develop 脚本。可理解为**对更新流程自身做一次「检查更新」**。

| 子步 | 做什么 |
|------|--------|
| 10.1 复盘 | 回看哪里第一次做就失败、哪里需要问用户、哪里返工，逐条追根因 |
| 10.2 校验 | 核对 `AGENTS.md` / `SECURITY.md` 等文件的断言是否仍成立（含外部链接可达性） |
| 10.3 归属 | 按可移植性把教训写到通用技能 / 适配层 / `AGENTS.md` / `SECURITY.md` |
| 10.4 体验 | 复盘「让用户等了几轮」，把能自行查证的收敛掉、能合并的步骤合并 |
| 10.5 证据 | 规范改动须**可复现、可追溯、允许质疑**；禁止凭印象改规范、把偶发当规律 |
| 10.6 产出 | 教训落到归属位置 + 规范修正一并提交 + 维护日志记录流程改进 |

> **硬性禁止**：凭印象改规范、把一次偶发当规律、为已写对的内容「再优化」、
> 删掉看起来没用但仍有约束力的条目——除非能证明其前提已消失。

**实测战果**：本步首次执行即发现 `SECURITY.md` 指向子仓 `SECURITY.md` 的
**死链**（子仓根本没建该文件），以及 12 处 `Asus-linux/asusctl` 失效链接
（项目已迁至 `OpenGamingCollective/asusctl`）。这些都不会产生构建错误，
只有主动审计才能发现。
## 使用
