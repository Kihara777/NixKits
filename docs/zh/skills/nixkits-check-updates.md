...
> NixKits 仓库的**软件包更新适配层**——在通用技能 `nix-flake-update-check` 之上，补充本仓库特有的四语文档同步、dsh 内置插件清单同步、维护日志记录与历史事故教训。
...
| 技能 | 职责 | 可移植性 |
|------|------|---------|
| `nix-flake-update-check` | 通用方法：包发现、包型分流的 hash 流程、flake.lock 处置、补丁内版本检查、nixpkgs 漂移陷阱 | 任意 nix flake 仓库 |
| `nixkits-check-updates` | 本仓库适配：四语文档、插件清单、维护日志、历史事故教训 | 仅 NixKits |
