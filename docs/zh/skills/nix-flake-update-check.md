...
| 路径 | `skills/nix-flake-update-check/`（`SKILL.md` + `builders.md` + `traps.md`）|
| 定位 | **通用**（不绑定任何具体仓库） |
...
单文件曾达 918 行，执行时难以定位。按「独立数据拆为配套文件」拆分后：

| 文件 | 内容 | 何时读 |
|------|------|--------|
| `SKILL.md` | 交互式澄清 + 第 1~9 步主流程 + 适配层契约 | 始终 |
| `builders.md` | 按 builder 的 hash 更新流程、`flake.lock` 处置 | 第 4 步 |
| `traps.md` | nixpkgs 漂移陷阱、fail-closed 校验、外链审计、Actions 更新、补丁内版本 | 第 7 步自检命中时 |

**第 7 步新增「提交前六问自检」**：多变体？依赖表一致？取源仍有效？真的运行过？文档表述仍成立？`flake.lock` 该提交吗？——六问全部提炼自同日实测的事故，命中后再按 `traps.md` 深入，替代通读全文。

**收尾由适配层补第 10 步**（本技能自身止于第 9 步）：流程复盘与规范校验，由适配层技能定义（NixKits 见 `nixkits-check-updates` 的「第 10 步（收尾）」）。其中特别要求**测试分支上产生的通用教训须当场搬回 main**——分支按约定不合并，教训不能随之滞留。
