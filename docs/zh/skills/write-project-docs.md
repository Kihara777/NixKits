...
[中文](write-project-docs.md) | [English](../../en/skills/write-project-docs.md) | [日本語](../../ja/skills/write-project-docs.md) | [ｶﾀﾘｯｼｭ](../../katalish/skills/write-project-docs.md)
...
- ｶﾀﾘｯｼｭ 语言支持 — 调用 translate-katakana 技能生成片假名英语翻译
- 编写分类 README（含语言切换器）
- 编写模块文档（基本信息表 + 安装 + 引用）
...
更新顺序：中文基准 → 英文翻译 → 日文翻译 → ｶﾀﾘｯｼｭ翻译。
四语列名映射：`基本信息` → `Info` / `基本情報` / `基本情報`、`功能` → `Features` / `機能` / `機能`。
ｶﾀﾘｯｼｭ（katalish）规则：半角片假名逐词机械替换英文文本，保留 Markdown 语法与代码块。技术术语使用内置词典，未命中时按音节音译。
...
- 所有语言必须包含基本信息表格（中文 `## 基本信息`、英文 `## Info`、日文 `## 基本情報`、ｶﾀﾘｯｼｭ `## 基本情報`）
