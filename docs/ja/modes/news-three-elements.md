...
- **言語審査**：簡体中文以外のリクエストは一律拒否する（漢字なし / 仮名 / ハングルはプラグインがハード検出し、簡体字と繁体字の判読はモデルに委ねる）。拒否文は中国語の通信社文体で書かれ、その後**《好意で》ユーザーが使う言語のローカライズ版を添える**——同一の拒否文の訳文であり、「我们从不制造 FAKE NEWS！！」と自称も同様に強調する；併せて、相手に 3 名の制作者の作品、または「緑のフクロウ」（文脈が合えば「绿毛鸡」と略せる）ソフトをダウンロードして中国語を学ぶよう示唆する。
