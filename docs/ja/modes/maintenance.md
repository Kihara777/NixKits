...
> NixOS模式を基盤とするリポジトリ保守専用エージェント：ドキュメント作成・保守ログ技能と上流更新チェック技能を注入し、**二層構成**の保守ワークフロープロンプト——汎用の方法＋本リポジトリ（NixKits）向け適配層——を読み込む。
...
- **保守ワークフロープロンプトは二層構成**（上記スキルと同じ分け方：汎用の方法 ← リポジトリ適配層）：

  | セクション名 | 適用範囲 | 内容 |
  |------|--------|------|
  | `maintenance-workflow`（順序 901） | **汎用**——どのリポジトリでも成立 | 論理カテゴリごとの分割コミット、push 後の記録、ドキュメントとコードの同期、修正の技能への汎化、技能内容の単一ソース |
  | `maintenance-workflow-repo`（順序 902） | **本リポジトリ（NixKits）の約束** | 四言語とその基準（`docs/zh/` を先に）、`write-maintenance-log` を準則とすること、項目数の一致という検証可能な判定、技能ツリーの単一ソース |

  判定はただ一つ：**この規約は他のリポジトリでも成立するか？** 成立すれば汎用層に残し、しなければ適配層へ。
  汎用層には本リポジトリ固有の名前は一切現れない（NixKits / 四言語 / `translate-*` / `MAINTENANCE.md` / `grep -c` / `docs/zh`）。

  コンポーネントオプション `repoWorkflow: false` で汎用層のみを残せる——別のリポジトリを保守するセッション向け。
- その他（システム検証、`nixos_shell` / `nixos_cli`、開発プロンプト、NixOS模式が同梱する 5 スキル）は NixOS模式と同一。
