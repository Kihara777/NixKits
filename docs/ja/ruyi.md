...
> * ruyi はクロスコンパイル不可（Python postPatch がターゲットプラットフォームのバイナリ実行を試行 + C 拡張依存）。NixKits CI は riscv64 キャッシュを提供しません。riscv64 上で `nix build` によりネイティブビルドしてください。バッジはワークフロー全体の状態を表示します。
