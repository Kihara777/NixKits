...
**Summary**：godot-ai 3.2.5 → 4.1.0 breaking-upgrade field test — On a test branch, `godot-ai` was rolled back to **3.2.5, its state before the breaking update**, as a baseline, and the skill was re-run to audit how it handles a cross-major fail-closed contract. **What the upgrade involved**: `packages/godot-ai.nix` went to 4.1.0 and gained a `postPatch` relaxing upstream's `setuptools==84.0.0` build-time pin (nixpkgs ships 83.0.0), completed the nine runtime dependencies (previously only six), and added `dontCheckRuntimeDeps`; a new `overlays/godot-ai-v4-deps.nix` raises mcp / pydantic(+core) / starlette / uvicorn / websockets to the versions upstream's exact pins require; **both** `flake.nix` and `overlays/default.nix` were switched to chained overlays. **Verification**: `godot-ai --version` → 4.1.0 (the startup fail-closed check passes, exit 0); `nix flake check` fully passes. **Reproducibility cross-check**: the six hashes **independently re-derived** in this run (five package sources plus pydantic-core's cargoDeps) are **identical one by one** to the values already merged on main, showing this upgrade path reproduces reliably. **Field-test conclusions about the skill**: (1) step 3 produced no false positives (7 packages, only 2 genuine UPDATEs); (2) interactive clarification worked — the godot-ai strategy (4 options) was settled before starting, with **no guessing along the way**; (3) trap 5 worked — after the build succeeded the binary was actually run to verify the fail-closed contract; (4) the "dual overlay landing sites" lesson recorded in the previous round **did not recur**: all three sites (`godotPkgs` / `overlays/default.nix` / overlays output registration) were changed in one pass, and `diff <(git show origin/main:...)` confirmed each was empty — evidence the record is effective. **New improvement produced by step 10's retrospective**: step 5's doc sync went **beyond mechanical substitution** — the dependency table's old "≥ ranges" wording had become a **factual error** against v4's exact pins (and the entry count was 6 vs the actual 9), so it was rewritten as an "upstream requirement vs nixpkgs provides" comparison table; on that basis the generic skill's step 5 gained a trigger criterion for "**when docs must be rewritten rather than mechanically substituted**". That change was checked against 10.5's evidence discipline: **reproducible** (`grep -A15 '^dependencies' packages/godot-ai.nix` yields 9 entries while the doc's original table had 6 — anyone can re-run it), **traceable** (it actually happened in this field test, commit `db5b7cc`), and **open to challenge** (the 10.3 attribution analysis voluntarily stated the case against generalising — doc quality is the doc skill's job — and scoped the rule accordingly: it governs "when to rewrite" and explicitly disclaims "how to write"). All four language docs synced
...
| 　 | runtime deps | 6 entries of "≥ ranges" → 9 fail-closed exact pins |
| 　 | new overlay | `overlays/godot-ai-v4-deps.nix` |

> **Note**: this entry lives on the test branch `test/skill-validation-godot-ai-v4` and **is not merged into main by agreement** (godot-ai on main became 4.1.0 back in `2a06bbf`). It is recorded to preserve the method and evidence of the skill field test.
