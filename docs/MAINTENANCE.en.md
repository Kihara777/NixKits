...
**Summary**：blender-mcp 1.0.0 → 1.0.3 — The improved update skill was validated in practice on a test branch, which surfaced one gap in the skill and fixed it. **Positive validation of the skill**: (1) the step 3 version check produced **no false positives** (the same command in the previous round reported 6 bogus UPDATEs across 8 packages, because it did not read each package's real upstream source; with that fixed, 7 packages yield exactly 1 genuine UPDATE); (2) step 9's chained check behaved correctly (precondition checks + the field-level follow-up criterion); (3) the multi-variant check worked (confirming blender-mcp is a single definition file with no codewhale-style dual-variant risk); (4) trap 5, "a successful build is not a working binary", was actually executed (ran `--help` to confirm). **The gap this run exposed**: the `/archive/<rev>.tar.gz` path generated by `fetchFromGitea` returns **403 for every tag**, while `/api/v1/repos/lab/blender_mcp/archive/<rev>.tar.gz` returns 200 — the root cause being that this fetcher **delegates to `fetchFromGitHub`** (the source is a single `makeOverridable` wrapper mapping `domain` to `githubBase`), so the path is GitHub-shaped and a self-hosted Gitea need not accept it. **The key trap**: "1.0.0 still builds" is an **illusion** — that source has long been in Cachix / the Nix store, so `nix build` hits the cache and **issues no request at all**; only `--rebuild` reveals it. **A working old build therefore does not imply the old path is usable.** **Fix**: switch to `fetchzip` against the API endpoint with `stripRoot = true` (reproducing fetchFromGitea's stripping of the top-level directory so `preConfigure = "cd mcp"` still holds — measured: the wrong `stripRoot` value turns the path into `blender_mcp/mcp` and the build fails). **Evidence (reproducible)**: (1) an HTTP status matrix over 4 tags × 2 paths (archive all 403 / api all 200); (2) the old approach failing with `curl: (22) ... 403` and `cannot download source`; (3) the new approach successfully producing the source; (4) output checks — `bin/blender-mcp` present, the addon shipped, `METADATA` Version 1.0.3; (5) **CI proof** (run `35183624795`), whose log shows the build phase actually running rather than `copying path ... from cache`, confirming the new source fetch genuinely works in a clean environment. **Generalisation**: by step 10's 10.3 criterion ("would this still hold in another nix flake repository?"), the generic skill gains a "non-GitHub sources (self-hosted forges such as Gitea)" section with a decision matrix (all tags 403 vs a single tag 403 vs UA-based anti-scraping), the `stripRoot` semantic trap, and a method for comparing old and new fetcher layouts, plus a "self-hosted forge" entry in the per-package-type section; the adapter records this repo's handling. All four language docs synced (version numbers + badge labels + feature lists)
...
| 　 | fetch method | `fetchFromGitea` (403) → `fetchzip` + `stripRoot = true` |
