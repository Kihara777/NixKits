...
- 面板頁面級 overlay（document 級 fixed portal、会話区域 clip 不、横屏/窄幅均画面外出不）；高度「锚点上方可用空間」自動 clamp（手機横屏 top bar 遮蔽回避）；横向可用幅不足時内容適応幅維持、面板横 scroll 表示（此際横 gesture 面板 scroll 優先、頁面切替上方指示 dot 経由），縦向同面板自身 scrollbar 表示。
