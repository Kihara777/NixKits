...
> NixKits 倉庫 的 **包更新適配層**——汎用技能 `nix-flake-update-check` 之上、本倉庫特有 四言語文書同期、dsh 内蔵插件一覧同期、保守記録記入、過去 事故教訓 補完。
...
| 技能 | 責務 | 移植性 |
|------|------|--------|
| `nix-flake-update-check` | 汎用手法：包検出、builder 別 hash flow、flake.lock 扱、修正内蔵版確認、nixpkgs 漂移 罠 | 任意 nix flake 倉庫 |
| `nixkits-check-updates` | 本倉庫適配：四言語文書、插件一覧、保守記録、過去 事故教訓 | NixKits 専用 |
