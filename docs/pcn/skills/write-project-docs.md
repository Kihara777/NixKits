...
- 計画之メタデタ評価部品情報抽出
- 機能別部品分類（基盤/服務/プロキシ/スキル）
- `docs/{zh,en,ja}/` ディレクトリ構造生成
- `skills/translate-*/` 之 `translate-*` 命名規則言語拡張之自動検出
- 分類 README 作成（言語切替付）
- 部品モジュル文書作成（基本情報表 + 導入 + 参照）
- 統一雛形スキルテンプレトスキル文書作成（基本情報 → 機能 → 使用）
- サブサブ代理サブエジェント部品カテゴリ別之並列化対応
...
- 目標行数 ~40-60 行。パッチ/部品モジュル文書 4 セクション標準（基本情報 → 修正内容 → 導入 → 注意）
...
| 方向 | メカニズム |
|------|-----------|
| 文書生成 → 翻訳スキル | `skills/translate-*/` スキャン、各 SKILL.md 之 frontmatter `language_code`/`display_name`/`base_language` 読取 |
| 翻訳スキル → 本計画 | 各スキル SKILL.md 内「他之スキル之関係」表宣言、呼出チェン明示 |
| 言語コド → パス | `language_code` → ディレクトリ名・・書類拡張子；`display_name` → 言語切替ラベル |

翻訳スキル之文書自身本雛形従、ルプ閉：文書生成 → 翻訳呼出 → 翻訳スキルスキル文書生成。

## 使用

ユザ「文書作成」「NixKits スタイルスタイル文書生成」依頼起動。
