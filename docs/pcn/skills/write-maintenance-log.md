...
> NixKits 規約基 MAINTENANCE.md 之執筆更新。軟体更新修正之両方対応、全言語同期。

## 自動発見契約

`translate-*` 命名規則言語拡張検出：`skills/translate-*/` 走査、各 SKILL.md 之 frontmatter （`language_code` / `display_name` / `base_language`）読取、多言語同期利用可能言語登録。
...
- 軟体更新記録之作成（概要 + ID 表 + 版表）
- 修正記録之作成（概要 + ID 表）
- 之 全言語同期（zh/en/ja/katalish/pcn）
