...
- **読取専用**：閲覧系工具限定掛載（`read`、`read_image`、`glob`、`grep`、`web_search`、`web_fetch`、`skill`、`ask_user_question`）；`write` / `edit` 雖 工具表掲載、呼出 一律 `readonly-gate` 通信社文体 拒否（許可一覧外 既定拒否故、将来追加工具 亦誤通過無）。読取 範囲 限定：絶対 path **工作区・添付目録・`/tmp`** 配下 唯許可、範囲外 拒否（相対 path 文件系統側 解決 委譲）。
