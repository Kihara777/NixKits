...
codewhale v0.9.3 既定 `sudo` 遮断。 [codewhale-sudo 補丁文書](codewhale-sudo.pcn.md) 参照。

## 既知問題

> ⚠️ **riscv64 源構築**: 上流 v0.9.3 以降 riscv64 予構築二進削除。NixKits `rustPlatform.buildRustPackage` 経由源交叉編輯提供。此実験的機能、初回 CI 依存 hash 不一致可能 — 後続 CI 検証修正予定。
...
> ⚠️ **v0.9.3 sudo 不能**: `no_new_privs` 標幟完全遮断 sudo（非暗号問題、Wheel NOPASSWD 無効）。上流報告済 — 修正待。
