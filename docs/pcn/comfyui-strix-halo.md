...
| 対応 GPU | gfx1151（Strix Halo）— ROCm 7.2 以本来支援 |

## 修正内容

- **ROCm 7.2 stable wheel**: torch 2.12.0 / torchvision 0.27.0 / torchaudio 2.11.0 追加
- **版自動選択**: rocm72 定義有場合 7.2、無場合 7.1 後退
- **rocmGfxOverride 選項**: 未対応 GPU 構造上書（`HSA_OVERRIDE_GFX_VERSION`）
- **xformers 自動無効化**: nixpkgs xformers ROCm 後端非対応故
- **C 構築道具連**: `stdenv.cc`、`binutils`、`gnumake` 注入、`CC=gcc` 設定、ComfyUI Manager 定制節点編集対応
...
- ROCm 7.2 gfx1151 本来支援済、`HSA_OVERRIDE_GFX_VERSION` 不要
