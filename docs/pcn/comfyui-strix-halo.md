...
| 対応 GPU | gfx1151（Strix Halo）— ROCm 7.2 ネイティブネイティブ支援 |

## 修正内容

- **ROCm 7.2 stable wheel**: torch 2.12.0 / torchvision 0.27.0 / torchaudio 2.11.0 追加
- **版自動選択**: rocm72 定義 7.2、 7.1 フォルバック
- **rocmGfxOverride オプション**: 未対応 GPU アキテクチャ之上書（`HSA_OVERRIDE_GFX_VERSION`）
- **xformers 自動無効化**: nixpkgs 之 xformers ROCm バックエンド非対応之
- **C 構築ビルド道具チェン**: `stdenv.cc`、`binutils`、`gnumake` 注入、`CC=gcc` 設定 ComfyUI Manager 之カスタムノドコンパイル対応
...
- ROCm 7.2 gfx1151 ネイティブネイティブ支援済之、`HSA_OVERRIDE_GFX_VERSION` 不要
