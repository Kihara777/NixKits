...
AMD Strix Halo（gfx1151 / RDNA 3.5 APU）向 ComfyUI ROCm 加速。
...
| 修正 | 本倉庫 `patches/comfyui-nix-strix-halo.patch` |
| 対応 GPU | gfx1151（Strix Halo）— ROCm 7.2 以原始支援 |

## 修正内容

- **ROCm 7.2 stable wheel**: torch 2.12.0 / torchvision 0.27.0 / torchaudio 2.11.0 追加
- **版自動選択**: rocm72 定義有場合 7.2、無場合 7.1 降級
- **rocmGfxOverride 選項**: 未対応 GPU 架構之上書（`HSA_OVERRIDE_GFX_VERSION`）
- **xformers 自動無効化**: nixpkgs 之 xformers、ROCm 後端非対応故
- **C 構築道具連**: `stdenv.cc`、`binutils`、`gnumake` 注入、`CC=gcc` 設定、ComfyUI Manager 之自定節点編訳対応
