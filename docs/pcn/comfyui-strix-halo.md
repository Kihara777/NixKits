...
- **版自動選択**: rocm72 定義 7.2、 7.1 フォールバック
- **rocmGfxOverride オプション**: 未対応 GPU アーキテクチャ之上書（`HSA_OVERRIDE_GFX_VERSION`）
- **xformers 自動無効化**: nixpkgs 之 xformers  ROCm バックエンド非対応之
- **C ビルドツールチェーン**: `stdenv.cc`、`binutils`、`gnumake` 注入、`CC=gcc` 設定 ComfyUI Manager 之カスタムノードコンパイル対応
...
- ROCm 7.2  gfx1151 ネイティブサポート済之、`HSA_OVERRIDE_GFX_VERSION` 不要
...
- モジュル自動 `amdgpu.gttsize=131072` 設定（Strix Halo UMA 用）
