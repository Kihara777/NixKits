...
ruyi  NixOS ランタイム互換性提供：プリコンパイル RISC-V ツールチェーンバイナリ、期待動的リンカパス `/lib64/ld-linux-x86-64.so.2`  NixOS 存在、之実行。之 overlay パッチ通之問題透過的処理。
...
- **ツールチェーンサブプロセス修復**：GCC 内部之 `cc1`、`as`、`collect2` 等之サブプロセス `patchelf` 自動修復
- **Nix console_scripts 互換**：`RUYI_ARGV0`  `exec -a` 之動作回復

## 設計

最小限之侵襲性：パッチ NixOS 環境検出、プリコンパイルツールチェーン `execv` 與場合之 ruyi 内部作動。NixOS 以外之環境パッチロジック完全短絡。
