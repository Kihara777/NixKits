...
**摘要**：語言切替標籤規則汎化 — display_name 意味語言自称修正、語言名非本地化規則 write-project-docs / translate-katalish / translate-pseudocn 追加；zh/katalish/pcn 全文書切替部残留本地化名修正

| 提交 | 説明 |
|------|------|
| `f5aee43` | docs(skill): write-project-docs — 語言名非本地化規則追加 |
| `7ba8c1d` | fix(katalish): 語言切替部 English 片仮名変換防止修正 |
| `5ce9f7d` | fix: display_name 意味語言自称修正 — 切替標籤分離 |
| `aa8634b` | fix(docs): zh 文書切替部残留旧名称修正 + MAINTENANCE 翻訳補完 + translate-* 汎化 |
...
**概要**：codewhale 0.8.61 — 上游修复；mcp-searxng 1.6.0 — 上游修复
...
**概要**：ruyi — NixOS 兼容性补丁`patches/ruyi-nixos-compat.patch`透明处理预编译 RISC-V 工具链的动态链接器路径GCC 子进程 ELF interpreter 修复和 console_scripts argv0 问题
...
**概要**：ruyi — NixOS 模块`services.ruyi`声明式生成 `/etc/xdg/ruyi/config.toml` 与环境变量
...
**概要**：ruyi — 新增 devShell 支持`nix develop github:Kihara777/NixKits#ruyi` 即可进入环境
...
**概要**：ruyi 0.51.0-alpha.20260616 — RuyiSDK 包管理器新包Python / Poetry 构建ruff + mypy + 320 单元测试 + 52 集成测试全部通过
...
**概要**：技能系统重构 — translate-katakana→translate-katalish 重命名新增 translate-pseudocn偽中国語write-project-docs 与 write-maintenance-log 语言扩展自动发现文档代码五语映射表
...
**概要**：write-maintenance-log 技能 — 从 nixkits-check-updates 剥离为独立技能双入口设计记入维护记录 + 更新维护记录；flake.lock 同步 .gitignore 前置检测与三路分支逻辑
...
**概要**：llama-cpp-rocm — 尝试用 builtins.fetchurl 替代 flake input 动态获取版本已撤销方案不可用
...
**概要**：mcp-searxng 文档 — CodeWhale MCP 配置指南常见陷阱警告env 默认为 {}故障排查章节
...
**概要**：nixos-modern-cli 技能 — Nix Store 路径陷阱章节gh auth setup-git 硬编码路径失效的诊断与通用修复模式
...
**概要**：llama-cpp-rocm 模块 — 恢复 modelsPreset 支持nixpkgs 已移除命名空间迁移至 nixkits三语迁移指南
...
**概要**：技能文档 — 维护日志格式规则系列自动发现泛化描述性标题精确 git commit 时间戳禁止 T00:00:00 占位符
...
**概要**：opencode-telegram — KillMode 改为 process添加 TimeoutStopSec 防止关机挂起
...
**概要**：文档重构 — 本地化文件移入 docs/ 目录；MAINTENANCE.md 首次添加合列规则纯表格格式回填完整提交历史
...
**概要**：rcc-fix — NixOS 模块systemd 死锁修复
...
**概要**：技能文档 — 源变更后文档同步规范；comfyui-strix-halo C 工具链说明；hash 计算注意事项泛化；基本情報规则多语言统一
...
**概要**：技能系统 — SKILL.md 全面中文化；三语对称性检查规则
...
**概要**：nixos-modern-cli 技能 — POSIX 工具指南与 nix 二进制路径提示
...
**概要**：write-project-docs — 新技能按 NixKits 风格为任意项目编写多语言文档系统
...
**概要**：codewhale — stdenv 拼写修复；llama-cpp-rocm 文档修正移除内联链接使用 system.nix 完整预设；opencode-telegram 首次设置流程
...
**概要**：kitsfmt — 多项修复vendor 目录恢复幂等性原地安全性with→builtins.attrValues 转换--stdin 标志；rcc-fix — 重写为 D-Bus 热插拔检测；build — .vscode gitignore 范围修正
...
**概要**：llama-cpp-rocm — NixOS 模块systemd 沙箱覆盖；opencode-telegram — NixOS 模块声明式配置自动安装；rcc-fix — visible 属性修复；技能文档 — 动态发现措辞
...
**概要**：技能系统 — nixkits-check-updatesnixkits-skillsnixos-modern-cli 三大技能同步上线；llama-cpp-rocm 动态追踪说明
...
**概要**：文档 — README 节名重命名快速开始→添加包→软件License→许可
...
**概要**：mcp-searxng 文档 — SearXNG + lighttpd 反向代理完整 NixOS 配置
...
**概要**：llama-cpp-rocm — 移除 llama-cpp-ver flake 输入使用 nixpkgs 默认版本
...
**概要**：kitsfmt — 修复 match_ast! 宏语法错误简化 comments_before 函数修正 src 路径

||説明|
|------|------|
|`e731eb7`|fix(kitsfmt): 修正 kitsfmt.nix 中的 src 路径|
|`314732c`|fix(kitsfmt): 修复 match_ast! 宏不支持通配符的问题|
|`1667e1d`|fix(kitsfmt): 修复 match_ast! 宏语法错误简化 comments_before 函数|
...
|`a1d1d36`|feat(kitsfmt): 生成 Cargo.lock更新 kitsfmt.nix 使用 rnix AST 构建|
...
**概要**：opencode-telegram 0.21.2 — 上游修复及依赖更新
...
**概要**：codewhale 0.8.59 — 修复若干 TUI 渲染问题；mcp-searxng 1.4.0 — 新增 HTTP 传输模式
...
**概要**：codewhale 0.8.58 — 上游修复；mcp-searxng 1.3.4 — 上游修复
...
**概要**：codewhale 0.8.57 — TUI 新增；mcp-searxng 1.3.2 — 上游修复
...
**概要**：codewhale 0.8.55 — 上游修复；mcp-searxng 1.3.1 — 上游修复
...
**概要**：codewhale 0.8.53 — 上游修复；mcp-searxng 1.1.0 — 上游修复；opencode-telegram 0.21.1 — 上游修复
...
|軟件包|旧|新|
|--------|--------|--------|
|comfyui-strix-halo|补丁ROCm 7.2 wheels 内嵌|

---

## 2026-06-02T05:57:11+09:00

**概要**：codewhale 0.8.49 — 上游修复；mcp-searxng 1.0.4 — 上游修复；obs-bilibili-stream 2.1.0 — 上游修复；opencode-telegram 0.21.0 — 上游修复
...
**概要**：mcp-searxng v1.0.3 — 新包；opencode-telegram v0.20.5 — 新包
...
**概要**：llama-cpp-rocm — 新包动态追踪上游最新 Release
...
|軟件包|旧|新|
|--------|--------|--------|
|llama-cpp-rocm|动态构建时获取上游最新 Release|

---

## 2026-05-14T07:38:08+09:00

**概要**：kitsfmt — 新包自建 Nix 格式化器；obs-bilibili-stream v1.0.0 — 新包
...
|軟件包|旧|新|
|--------|--------|--------|
|rcc-fix|跟随 nixpkgsoverlay + patch|

---

## 2026-06-17T06:48:47+09:00

**概要**：fix(mcp-searxng): 修复入口文件错误 — dist/index.js → dist/cli.jsMCP 服务器可正常启动
...
**概要**：feat(llama-cpp-rocm): 恢复 modelsPreset 支持nixpkgs 已移除命名空间迁移至 nixkits

---

## 2026-06-10T04:31:20+09:00

**概要**：fix(opencode-telegram): 修改 KillMode 为 process添加 TimeoutStopSec 防止关机挂起
...
**概要**：fix(kitsfmt): 修复 inherit 逗号缩进字符串损坏lambda 空格等多个格式化问题；修复幂等性
...
**概要**：fix(rcc-fix): 用 D-Bus InterfacesAdded 事件替代轮询优化二合一键盘热插拔检测
