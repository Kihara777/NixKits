...
**概要**: codewhale 0.8.61 — 修正；mcp-searxng 1.6.0 — 修正
...
**概要**: ruyi — NixOS 互換性補丁`patches/ruyi-nixos-compat.patch` RISC-V 工具動的GCC ELF interpreter 修復console_scripts argv0 問題透過的処理
...
**概要**: ruyi — NixOS 模塊`services.ruyi``/etc/xdg/ruyi/config.toml` 環境変数宣言的生成
...
**概要**: ruyi — devShell 支持追加`nix develop github:Kihara777/NixKits#ruyi` 即時環境
...
**概要**: ruyi 0.51.0-alpha.20260616 — RuyiSDK 軟件包管理器新規軟件包Python/Poetryruff + mypy + 320 + 52 統合合格
