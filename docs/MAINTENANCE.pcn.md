...
**摘要**：refactor(preset): 維護模式 prompt を「汎用方法 + 本倉適配層」に分割 —— `maintenance-skills` 従来 NixKits 工作流 を**公開預設 に 丸 ごと 硬编码**、`skills/` 既存 分法（`nix-flake-update-check` 汎用 ← `nixkits-check-updates` 本倉適配）と 矛盾。現在 二 段：`maintenance-workflow`（序号 901、汎用——如何倉庫 皆 成立：論理類別 毎 分割 commit、push 後 記録、文書 と code 同期、修正 技能 汎化、技能内容 単一來源）と `maintenance-workflow-repo`（序号 902、本倉 約束：四言語 と `docs/zh/` 基準、`write-maintenance-log` 準則、項目数 一致 之 検証可能 判据、技能 tree 単一來源）。判据 唯一「此規約、換倉庫 尚 成立 乎」。汎用層 に 本倉専名 一切 不出現（NixKits / 四言語 / `translate-*` / `MAINTENANCE.md` / `grep -c` / `docs/zh`、逐詞 照合 済）。新 component 選項 `repoWorkflow: false` 汎用層 のみ 残存 可。四言語 文書 同期
...
**摘要**：docs(skill): 「起動器設定 命令式改変 不可」事故を `nixos-specialisation-tuning` に記録 —— 本件は**我 導入、実際 系統 起動不能 至**障害。**要求**：上流 nixpkgs は Limine 既定項目を「specialisation 存在すれば 第三番 選択」と**硬编码**（`limine-install.py:533`）。故 選択面 存在するのみで既定 選択面 至。既定 戻す為、`extraInstallCommands`（nixpkgs が install script の**後** 実行 保証）で `limine.conf` の `default_entry` 行 を `sed` 改写。**失敗機構**：本 鉤子 は Limine が**設定 哈希 固化 後** 実行 —— `limine-install.py:660` は `enrollConfig=true` 時 `blake2b(config_file.strip())` 計算、`limine enroll-config` で EFI 二進 固化。`limine.nix:471` の `${install} "$@"` 固化 完了、`:472` の `${extraInstallCommands}` が 設定 改写。固化後 改写 → **哈希 不一致** → Secure Boot 下 起動器 設定 読込 拒否 → **系統 起動不能**。復旧 外部鏡像 で secure boot と `panicOnChecksumMismatch` 手動 無効化 必要。**一般化形式**：`extraInstallCommands` 如 き「install 後」鉤子 は 安全 最終修正点 に見える。然 し install 内部 で**既 検証・固化 完了** あり得る —— 「鉤子 最後 実行」≠「鉤子 変更 全検証 通過」。**技能 二節 追加**：① `### 引导菜单与默认面` —— 一般規則（起動器設定 宣言式 必須、生成物 sed/cp 改写 不可）、事故 完全 記録 と 失敗機構、変更前 正 調査順序（install script 全 flow 読、 「file 書込 → 検証/署名」 順序 全 洗出、変更 固化前 落 確認。安全 入口 無 場合 上流 論理 従、対抗 せず）、固化後 変更 避 得 ない 場合 は**上流 と byte 単位 同一** 算法 で 再固化 必要、及 び 安全 switch（`enrollConfig`/`validateChecksums`/`panicOnChecksumMismatch`、何れ も nixpkgs が "allows bypassing secure boot" 断言） ダウングレード 規律；② 面切替後 二 運行級 障害 —— `### 分面切换后的运行级收敛`（`switch-to-configuration` は active target 変更 せず。判定 に `is-active` を**使用 不可**。systemd target 加算的 で `graphical` は `multi-user` 要求、故 常 真。`default.target` の**解決値** 使用。尚 `systemctl get-default` は `default.target` 自体 返、其 指 先 非 ず）と `### 用户级 systemd 实例跨面陈旧`（`user@<uid>.service` は 面 跨 でも 再起動 されず、unit link 失効。`daemon-reload`/`daemon-reexec` 何れ も 無効、instance 全体 再起動 必要。逆方向 清理 と 検証判定基準 含 —— **「合成器 存在」を「桌面 正常」と 見做 不可**。黒画面 時 こそ 合成器 動作 中）。frontmatter `description` と「适用场景」同期更新、技能 正 路由 起動 可能 に
