...
**概要**：blender-mcp 1.0.0 → 1.0.3 — テストブランチ上で改良後の更新スキルを実戦検証し、スキルの空白を一つ露呈させて修正した。**スキル有効性の肯定的検証**：①第 3 步の版照合が**偽陽性ゼロ**（前回の同一コマンドは 8 パッケージに対し 6 件の誤 UPDATE を出した。根因は各パッケージの実際の上流ソースを読んでいなかったこと。修正後は 7 パッケージで真の UPDATE が 1 件のみ）；②第 9 步の連鎖チェックが正常（前提検証 + フィールド単位の追従判据）；③多変体チェックが有効（blender-mcp は単一定義ファイルで、codewhale 型の双変体リスクがないことを確認）；④罠 5「ビルド成功 ≠ 使用可能」を実行（実際に `--help` を走らせて確認）。**今回露呈したスキルの空白**：`fetchFromGitea` が生成する `/archive/<rev>.tar.gz` 経路が**全 tag で 403** を返し、`/api/v1/repos/lab/blender_mcp/archive/<rev>.tar.gz` は 200 を返す——根因はこの fetcher が **`fetchFromGitHub` に委譲**していること（ソースは `domain` を `githubBase` に写す一层の `makeOverridable` のみ）。経路が GitHub 形式であるため、自ホストの Gitea が受け入れるとは限らない。**重要な罠**：「1.0.0 はまだビルドできる」は**錯覚**である——そのソースは既に Cachix / Nix store にあり、`nix build` はキャッシュに当たって**一切リクエストを発行しない**。`--rebuild` によって初めて露呈する。ゆえに**「旧版がビルドできる」ことから旧経路が使えると推論してはならない**。**処置**：API エンドポイントに対して `fetchzip` + `stripRoot = true` へ変更（fetchFromGitea の最上位ディレクトリ剥離の意味論を再現し、`preConfigure = "cd mcp"` を成立させる——実測では `stripRoot` の値が誤ると経路が `blender_mcp/mcp` になりビルドが失敗する）。**証拠（再現可能）**：①4 tag × 2 経路の HTTP 状態行列（archive 全 403 / api 全 200）；②旧方式のビルドが `curl: (22) ... 403` と `cannot download source` を報告；③新方式が source の生成に成功；④成果物照合で `bin/blender-mcp` が存在、addon が同梱、`METADATA` の Version が 1.0.3；⑤**CI 実証**（run `35183624795`）のログが `copying path ... from cache` ではなく実際のビルド段階の実行を示し、新たなソース取得がクリーン環境で真に機能することを証明。**汎化**：第 10 步の 10.3 判据（別の nix flake リポジトリへ移しても成立するか）により、汎用スキルに「非 GitHub ソース（Gitea 等の自ホスト forge）」の節を新設。判定行列（全 tag 403 / 単一 tag 403 / UA による反爬）、`stripRoot` の意味論の罠、新旧 fetcher のレイアウト比較による検証方法を含み、パッケージ型の分節にも「自ホスト forge」の入口を追加。適応層には本リポジトリの処置を記録。四言語の文書を同期（版番号 + バッジラベル + 機能一覧）
...
| 　 | ソース取得方式 | `fetchFromGitea`（403）→ `fetchzip` + `stripRoot = true` |
