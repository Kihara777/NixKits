...
**概要**：godot-ai 3.2.5 → 4.1.0 破壊的アップグレードの実戦テスト — テストブランチ上で `godot-ai` を**破壊的更新前の 3.2.5** へ戻して基線とし、スキルを再実行してクロスメジャーの fail-closed 契約への対処を監査した。**更新の内容**：`packages/godot-ai.nix` を 4.1.0 へ上げ、上流の `setuptools==84.0.0` ビルド時 pin を緩和する `postPatch` を追加（nixpkgs は 83.0.0）、実行時依存を 9 個へ補完（従来は 6 個のみ）、`dontCheckRuntimeDeps` を追加；新規 `overlays/godot-ai-v4-deps.nix` が mcp / pydantic(+core) / starlette / uvicorn / websockets を上流の厳密固定が要求する版まで引き上げ；`flake.nix` と `overlays/default.nix` の**両方**を連鎖 overlay に変更。**検証**：`godot-ai --version` → 4.1.0（起動時の fail-closed 検証を通過、exit 0）；`nix flake check` は全て通過。**再現性の相互検証**：今回**独立に再導出**した 6 個の hash（5 パッケージの src + pydantic-core の cargoDeps）は、main に既にマージ済みの値と**一つずつ一致**し、この更新経路が安定して再現することを示す。**スキル実戦の結論**：①第 3 步は偽陽性ゼロ（7 パッケージ中、真の UPDATE は 2 件のみ）；②対話的確認が機能——着手前に godot-ai の戦略（4 択）を確定し、**邊做邊猜は発生しなかった**；③罠 5 が機能——ビルド成功後に実際に実行して fail-closed 契約を検証；④前回記録した「双 overlay の落とし所」の教訓は**今回再発しなかった**：記憶に従い三箇所（`godotPkgs` / `overlays/default.nix` / overlays 出力登録）を一度に変更し、`diff <(git show origin/main:...)` で各所が空であることを確認——記録が有効である証拠。**第 10 步の振り返りが生んだ新たな改善**：第 5 步の文書同期が**機械的置換を超えた**——依存表の旧い「≥ 範囲」という記述が v4 の厳密固定に対して**事実誤り**となっており（条目数も 6 対 実際は 9）、「上流の要求 vs nixpkgs の提供」の対照表へ書き直した；これに基づき汎用スキルの第 5 步に「**文書を機械的置換ではなく書き直すべき時**」の触发判据を新設。この変更は 10.5 の証拠規律で検証済み：**再現可能**（`grep -A15 '^dependencies' packages/godot-ai.nix` は 9 項、文書の元表は 6 項——誰でも再実行できる）、**追跡可能**（今回の実戦で実際に発生、コミット `db5b7cc`）、**異議申立て可能**（10.3 の帰属分析で汎化に**反対する理由**を自発的に記述——文書の品質は文書作成スキルの職責——そして範囲を限定：「いつ書き直すか」のみを定め、「どう書くか」は定めないと明記）。四言語の文書を同期
...
| 　 | 実行時依存 | 6 項の「≥ 範囲」→ 9 項の fail-closed 厳密固定 |
| 　 | 新規 overlay | `overlays/godot-ai-v4-deps.nix` |

> **注**：本条はテストブランチ `test/skill-validation-godot-ai-v4` 上にあり、**取り決めにより main へはマージしない**（main の godot-ai は `2a06bbf` で既に 4.1.0）。スキル実戦の手法と証拠を残すために記録する。
