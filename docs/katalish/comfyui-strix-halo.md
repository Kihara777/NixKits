...
| Patch | This repo `patches/comfyui-nix-strix-halo.ﾊﾟｯﾁ` |
...
- **Auto ﾊﾞｰｼﾞｮﾝ selection**: prefers 7.2 ｳｪﾝ rocm72 definitions exist, falls back to 7.1
- **rocmGfxOverride option**: override ﾌｫｱ unrecognized GPU architectures (`HSA_OVERRIDE_GFX_VERSION`)
- **Auto --disable-xformers**: nixpkgs xformers lacks ｱ ROCm backend
- **C ﾋﾞﾙﾄﾞ toolchain**: injects `stdenv.cc`, `binutils`, `gnumake`, sets `CC=gcc` ﾌｫｱ ComfyUI Manager custom node compilation
...
- xformers errors: ｻﾞ ﾊﾟｯﾁ auto-adds `--disable-xformers`
- The ﾓｼﾞｭｰﾙ auto-sets `amdgpu.gttsize=131072` ﾌｫｱ Strix Halo UMA
