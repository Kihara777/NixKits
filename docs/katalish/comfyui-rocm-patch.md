...
ROCm ﾌｨｰﾁｬｰ ﾊﾟｯﾁ ﾌｫｱ ComfyUI.

ｲﾝｸﾙｰﾄﾞ **Strix Halo (gfx1151 / RDNA 3.5 APU) exclusive optimizations**, tested ｵﾝ Ryzen AI MAX+ 395 / Radeon 8060S.

## ｵｰﾊﾞｰﾋﾞｭｰ

| Item | Value |
|------|-----|
| Type | ｵｰﾊﾞｰﾚｲ + NixOS ﾓｼﾞｭｰﾙ |
| Option | `nixkits.comfyui-rocm-patch.enable` |
| Location | `modules/comfyui-rocm-patch.nix` + `patches/comfyui-nix-strix-halo.patch` |
| Supported GPU | gfx1151 (Strix Halo) — natively recognized ﾊﾞｲ ROCm 7.1 |
...
- **C build toolchain**: Injects `gcc`, `binutils`, `gnumake` ｲﾝﾄｩ PATH, sets `CC=gcc`
...
Use upstream flake directly, overridden ﾊﾞｲ local module patch (recommended):
...
`cachix use nixkits` (auto-declared via `nixConfig` ｳｪﾝ used ｱｽﾞ ｱ flake input).

> ⚠️ ﾃﾞｨｽ entry ｲｽﾞ ｱﾝ ｵｰﾊﾞｰﾚｲ — it modifies upstream nixpkgs packages rather than being an independent build, ｱﾝﾄﾞ ｲｽﾞ ﾉｯﾄ ｲﾝ ｻﾞ binary cache.

## Notes

- ROCm 7.1 natively recognizes gfx1151; `HSA_OVERRIDE_GFX_VERSION` ｲｽﾞ ﾉｯﾄ required
- If GPU ｲｽﾞ ﾉｯﾄ detected, try `services.comfyui.rocmGfxOverride = "11.0.0"`
- xformers errors: ｻﾞ module auto-disables xformers via `--disable-xformers`
- Module auto-sets `amdgpu.gttsize=131072` (optimized ﾌｫｱ Strix Halo UMA)
- After C toolchain injection, ComfyUI Manager ｷｬﾝ build custom node dependencies online
