...
**ｻﾏﾘｰ**：docs(skill): add Nix Store path trap section to nixos-modern-cli
...
**ｻﾏﾘｰ**：docs(skill): enforce exact git commit timestamps, ban T00:00:00 placeholder
...
**ｻﾏﾘｰ**：feat(rcc-fix): add NixOS module for systemd deadlock fix
...
**ｻﾏﾘｰ**：docs(comfyui-strix-halo): add C build toolchain + CC=gcc to changes list
...
**ｻﾏﾘｰ**：feat(skills): add write-project-docs skill with trilingual docs
...
**ｻﾏﾘｰ**：fix(codewhale): fix stdenv typo causing build failure
...
**ｻﾏﾘｰ**：feat(llama-cpp-rocm): add NixOS module for service sandbox overrides
...
**ｻﾏﾘｰ**：fix(llama-cpp-rocm): remove llama-cpp-ver, use nixpkgs version directly
...
**ｻﾏﾘｰ**：fix(build): restrict .vscode gitignore to repo root to not exclude vendored crate files
