...
| dsh-alpha | DeepSeek Harness（DSH）— アルファ開発チャネル（0.1.6-alpha.2） | [docs/ja/dsh.md](ja/dsh.md) |
...
| ruyi<br>ruyi-beta<br>ruyi-alpha | RuyiSDK パッケージマネージャ（RISC-V 開発ツール）<br>stable 0.52.0 · beta 0.53.0-beta.20260917 · alpha 0.52.0-alpha.20260714 | [docs/ja/ruyi.md](ja/ruyi.md) |
...
| dsh-api-balance | API 使用量残高——webui の使用量リング（送信ボタン左）に「用量 / 残高」タブ切替を追加。残高・当日 / 当月 / 30 日消費とチャートを表示。プラットフォームトークンはデフォルトで本機ブラウザのログイン状態から自動スキャン取得（手動接続はフォールバック）。**独立[リポジトリ](https://github.com/Kihara777/dsh-api-balance)へ移転済み。本リポジトリは宣言的インストール用の薄いラッパーパッケージを保持** | [docs/ja/dsh-api-balance.md](ja/dsh-api-balance.md) |
...
セキュリティポリシー（**評価済みの外部報告一覧**を含む——解決済みの誤検出は再受理しません）は [`SECURITY.md`](../SECURITY.md) を参照。
