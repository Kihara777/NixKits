...
NixKits — 软件、补丁、ﾆｯｸｽOS 模块与 AI 编码助手的技能合集。
...
# 本地
inputs.nix-kits.ﾕｰｱｰﾙｴﾙ = "/home/kix/NixKits";
```
...
| kitsfmt | ﾆｯｸｽ 格式化器（AST 排序 + Best-Practice 自动修正） | [docs/zh/kitsfmt.md](docs/zh/kitsfmt.md) |
| mcp-searxng | SearXNG 的 ｴﾑｼｰﾋﾟｰ ｻｰﾊﾞｰ | [docs/zh/mcp-searxng.md](docs/zh/mcp-searxng.md) |
| obs-bilibili-stream | ｵﾌﾞｴｽ 的 ﾋﾞﾘﾋﾞﾘ 直播插件 | [docs/zh/obs-bilibili-stream.md](docs/zh/obs-bilibili-stream.md) |
| opencode-telegram | OpenCode 的 ﾃﾚｸﾞﾗﾑ ﾎﾞｯﾄ 客户端 | [docs/zh/opencode-telegram.md](docs/zh/opencode-telegram.md) |
| ﾙｲｰ | RuyiSDK 包管理器（RISC-V 开发工具） | [docs/zh/ﾙｲｰ.md](docs/zh/ﾙｲｰ.md) |
...
| 包 | `nix develop` |
|-----|---------------|
| ﾙｲｰ | `nix develop nix-kits#ﾙｲｰ` |

## 补丁

独立 ｵｰﾊﾞｰﾚｲ，不包含在 `default` 内：
...
| ruyi-nixos-compat | 为 ﾙｲｰ 提供 ﾆｯｸｽOS 运行时兼容（ELF interpreter 重定向 + GCC 子进程修复） | [docs/zh/ruyi-nixos-compat.md](docs/zh/ruyi-nixos-compat.md) |

## 技能

供 AI 编码助手使用的技能：

> 本项目的技能主要面向中文用户和中国开源模型，所有 ｽｷﾙ.md 均使用中文编写。
...
| nixos-modern-cli | ﾆｯｸｽOS 现代 CLI 操作指南（面向 AI 模型） | [docs/zh/skills/nixos-modern-cli.md](docs/zh/skills/nixos-modern-cli.md) |
| recover-nixos-config | 从 ﾆｯｸｽ store 恢复误删的 /etc/nixos 配置 | [docs/zh/skills/recover-nixos-config.md](docs/zh/skills/recover-nixos-config.md) |
