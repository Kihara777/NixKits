...
- **Runtime skills**: the `maintenance-skills` entry registers `write-project-docs`, `write-maintenance-log`, and `nixkits-check-updates` at apply time from the repo `skills/` tree **embedded at build time**, and auto-discovers every `translate-*` language extension — the repo `skills/` is the single source of truth, so a fresh session is always current.
