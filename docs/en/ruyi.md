...
> * ruyi cannot be cross-compiled (Python postPatch runs target-platform binary + C-extension deps). NixKits CI does not provide riscv64 cache. Build natively on riscv64 with `nix build`. Badge shows workflow-level status.
