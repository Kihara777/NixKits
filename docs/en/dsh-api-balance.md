...
- The panel is a page-level overlay (a document-level fixed portal, never clipped by the conversation area and never off-screen in landscape or narrow widths); its height clamps automatically to the space above the anchor (avoiding the top bar in phone landscape); when the horizontal space is too narrow, content keeps its adapted width and scrolls horizontally via the panel (horizontal gestures then scroll the panel, and pages switch via the indicator dots), and vertically via the panel's own scrollbar.
