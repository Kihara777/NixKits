...
| ruyi<br>ruyi-beta<br>ruyi-alpha | RuyiSDK 包管理器（RISC-V 開發工具）<br>stable 0.51.0 · beta 0.52.0-beta.20260824 · alpha 0.52.0-alpha.20260714 | [docs/pcn/ruyi.md](docs/pcn/ruyi.md) |
...
| dsh-api-balance | API 用量残高——webui 用量圓環（送信按鈕左）添加「用量 / 余额」標籤切替、余额視図示當前 API KEY 帳戶残高（DeepSeek /user/balance） | [docs/pcn/dsh.md](pcn/dsh.md) |
...
| 維護模式（id `maintenance`） | NixOS模式基盤；`write-project-docs`/`write-maintenance-log`/`translate-*` 技能与倉庫維護工作流注入 |
