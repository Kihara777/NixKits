...
- **依赖外部模块时必须添加 `assertions`**（参见 `comfyui.nix`）。
...
- **版本号必须与包定义一致**：`docs/<lang>/<pkg>.md` 的「版本」行（四语）与包内多通道表里的版本，都须等于 `packages/<pkg>.nix` 声明的版本；`develop/check-doc-versions.py` 已把这条固化为断言并挂入 `nix flake check`。**升级包时先改文档再提交**，否则检查失败。
  - 例外须在该脚本的 `EXEMPT` 中显式登记（如 `dsh-api-balance` 薄封装刻意不标版本）。
  - 脚本只校验**能从定义机械读出**的版本号；依赖表、平台支持等需人工核对的内容不在其范围（判断「何时该重写文档」见 `nix-flake-update-check` 技能第 5 步）。
...
**本仓库的所有开发与维护，纯粹由维护者（狐莉）与其代理（小爪）完成。
不引入任何外部的自动化集成。**
...
| 拒绝 | 理由 | 替代做法 |
|------|------|---------|
| 第三方 CI 扫描器（如 `ai-plugin-scanner-action`） | 会在我们的 runner 上**执行**不可审计的第三方代码 | 扫描由外部目录方自行进行；我们主动补全其指出的工程卫生项 |
| Dependabot（含纯配置的 `dependabot.yml`） | 即使它不可执行、只能开 PR、且拿不到 secrets，它仍是**外部自动化集成**，行为不由我们掌控 | **自行实现同等能力**（见下） |

**判据**：需要一项自动化能力时，先问"能否用仓库内已有的工具（`gh`、
`git`、`nix`）自行实现"。可以，就自己写进技能；不可以，就人工执行。

**已有实例**：把第三方 action 固定到 SHA 后，会失去更新通知——这处盲点
**不由 Dependabot 填补**，而是在 `nix-flake-update-check` 技能中自行实现
检查流程（`gh api` 查 tag → 取 commit SHA → 回写并同步注释版本号）。

**代价与接受**：action 的安全更新需要主动跑技能检查，而非自动收到 PR。
我们接受这一代价，换取"仓库内不运行任何外部自动化"的确定性。
