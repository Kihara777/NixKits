...
| 文件 | 内容 | 何时读 |
|---|---|---|
| **`SKILL.md`**（本文） | 交互式澄清 + 第 1~9 步主流程 + 适配层契约 | 始终 |
...
> ——这是固定 SHA 的固有代价，**任何采纳该做法的人都会继承它**：仓库维护者、
> 提交 PR 的贡献者、以及评估接手成本的人都一样。通知渠道关掉了，就必须有人
> 主动去查：若仓库同时还有「不引入外部自动化」的边界（托管平台不提供该服务、
> 或明确拒绝此类集成），那么**本技能就是唯一会发现它过期的途径**；即使平台
> 已启用某种自动化，也应跑一遍作**交叉核对**（那些工具只在其配置的版本通道
> 内推进，未必覆盖到你关心的那条线）。漏掉本节，这类更新就彻底无人过问。

> ⚠️ **只读、低成本、任何人可跑**：下面的发现命令只是读取 workflow 文件，
> 不修改任何东西。**发现 ≠ 必须升级**——若你只是路过（无写权限、或与手头
> 任务无关），在报告/PR 描述里指出即可，不必自行改动（升级属需评审的 CI
> 变更，处置建议见 `traps.md`「若你不是维护者：升级 action 也应走 PR」）。
...
**来源实例**（取自一个实际仓库，可用上面两条命令在该仓复现）：`packages/godot-ai.nix`
列出 **9** 个运行时依赖，而 `docs/zh/godot-ai.md` 的依赖表只列 **6** 个、且版本
...
| 8 | **`.github/workflows/` 里固定 SHA 的 action 查过了吗？** 见第 2 步末节与 `traps.md` | 固定 SHA 后收不到通知，**这类更新彻底无人过问**（来源仓库实测：3 个 action 共 6 处从未被检查） |

> **这八问的来源**：全部是**来源仓库**在 2026-09-17 ~ 09-20 期间**实际踩到的
> 坑**，每一问都对应一次真实返工或缺陷。**不是假想的检查表**——括号里的数字
> 是该仓库当时的实测记录，你的仓库不一定相同，判据本身才是重点。
>
...
按仓库约定记录本次更新（维护日志 / CHANGELOG / 无）。**具体用哪套由仓库适配层
指定**——例如采用维护日志的仓库会用其配套的记录技能；没有该约定的仓库则退化为
CHANGELOG 或直接跳过本步。
...
```markdown
**摘要**：<子项目名> <旧版本> → <新版本> — 薄封装 rev/hash 同步（子仓变更见
[子仓变更记录](https://github.com/<owner>/<sub>/blob/main/<日志文件>#<条目锚点>)）
```

> 锚点用子仓条目的**时间戳**（如 `#2026-09-17t11-21-34-09-00`）——它在该仓库内
> 唯一，且子仓与主仓都用同一套标题格式时可直接推导。**具体的推导规则与易错点
> 由仓库适配层给出**（它知道本仓用哪套记录格式）；给不出锚点就退化为给记录
> 文件链接 + 条目标题文本。

## 仓库适配层

通用流程覆盖到第 8 步为止（第 9 步是可选扩展）。**仓库特有的环节不由本技能
定义**，而由该仓库的适配层补充——通常是同名的「仓库适配层」文档或技能，
也可能只是仓库自己的一份约定说明。

**执行方式**：若当前仓库存在适配层，**先读取它**再执行本技能（不必假定宿主
提供"加载技能"的能力，能读到即可）；两者冲突时以适配层为准（适配层了解本仓库
的真实约定）。**没有适配层时**，按第 5 步（文档）与第 8 步（记录）给出的分支
自助处理，**不要臆造约定**——那两步都写了"无该结构则跳过"的退路。
