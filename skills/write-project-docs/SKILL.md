...
- 扩展语言（`skills/translate-*/` 自动发现）子目录文件：
  ...
  - 每个语言的标签使用该语言的 `display_name`（语言自称）
  - 当前语言自身不加链接（纯文本），其他语言附相对路径链接
  - 相对路径根据目录层级自动计算（同级 `../<lang>/`、上级 `../../<lang>/` 等）
  - ⚠️ **必须从源语言文件提取路径后用脚本逐个精确构造**，
    不可对切换器行做 sed 模式匹配替换（极易导致入口丢失或路径错位）
  - katalish：从英文源生成，自身标签 `ｶﾀﾘｯｼｭ`（纯文本）
  - pcn：从日文源生成，`日本語`→`[日本語]`(加链接)，自身标签 `偽中国語`（纯文本）
...
<!-- 多语言 README 中 <语言> 为当前语言代码（en/ja/katalish/pcn），根 README 为 docs/zh -->
...
- **语言切换器标签规则** —
  所有语言入口标签统一使用各语言的 `display_name`（语言自称）：
  `中文`、`English`、`日本語`、`ｶﾀﾘｯｼｭ`、`偽中国語`。
...
- **新增模块后必须同步所有 README** — 在主语言文档目录下添加新文档时，
  必须在根 README 及全部多语言 README（`docs/README.<code>.md`，按 `language_code` 枚举）
  的对应分类表中添加行。遗漏导致文档孤立。
- **多语言 README 的 Docs 列链接必须指向当前语言** — 显示文本 `[path]` 和
  链接目标 `](path)` 必须一致，均为 `docs/<code>/<module>.md`（`<code>` 为当前语言代码）。
  禁止硬编码主语言目录，应从当前多语言 README 的 `language_code` 推导。
- **翻译完成后验证全部链接** — 遍历所有 `](...)` 相对路径，确认目标文件存在。
  损坏模式包括：翻译引擎侵入代码块或链接路径、子目录前缀错误、显示文本与目标不一致。
- **切换器从源文件构造，禁止模式匹配替换** — 语言切换器必须从源语言文件
  提取路径后用脚本逐个精确生成。sed 模式匹配替换极易导致入口丢失或路径错位。
- **多语言维护日志（MAINTENANCE）从基线重建优于修补** — 当行数不一致或结构
  损坏时，从主语言基线复制骨架后替换 UI 字符串，比逐条修补更可靠。
