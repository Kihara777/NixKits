...
description: 按 NixKits 风格为任意项目编写或重新生成完整的多语言文档 — 简洁、表驱动、含语言切换器标题和模块分类 README。覆盖 README、各模块文档及技能文档；含「主仓薄封装 + 子仓完整文档」的迁出组件文档分工与子仓引用关系的显式记录。
---
...
| 文件 | 内容 | 何时读 |
|---|---|---|
| **`SKILL.md`**（本文） | 编写规则、双语切换器、自动发现契约、技能文档同步规则 | 始终 |
| [`templates.md`](templates.md) | 完整模板集：目录结构、模块文档模板、README 模板、维护日志模板 | 第一次搭建某类文档时 |
...
只要主仓**引用了同账户子仓**（不论是否「已迁出」，薄封装、flake input、
submodule 都算），文档里必须能找到这条引用关系——否则维护者会在主仓里
找不到源码，或重复实现子仓已有的东西。

| 位置 | 必须写明的信息 |
|------|---------------|
| 主仓短页「基本信息」 | **源码仓库** URL + **主仓侧角色**（薄封装 / input / submodule）+ 主仓钉住的 `rev` 或版本 |
| 子仓 README | 该项目的**完整文档**；若有下游引用方，可注明「被 <主仓> 以薄封装引用」 |
| 主仓完整文档（若该组件未迁出，仅被引用） | 说明引用方式与**同步方式**（如何升级钉住的坐标） |

**同步说明不可省**：读者需要知道「子仓发新版后，主仓这边要做什么」。
薄封装的答案通常是「改 `rev` + 重算两个 hash」，写清哪两个。

> 与维护日志的分工：文档说明**引用关系与同步方法**（长期有效的结构信息），
> 维护日志记录**每次坐标变更**（时间线上的事件）。两者都要有，内容不重复。
...
| 引用了子仓但不写引用关系 | 主仓短页写明源码仓库 + 主仓侧角色 + 钉住的坐标 + 同步方式 |
| 主仓维护日志复述子仓变更详情 | 主仓只记坐标变更 + 链接到子仓条目；详情留在子仓 |
