...
为一套**多语言文档体系**提供 pcn（伪中国语）语言支持。本技能**不绑定任何
具体项目**——任何采用「基准语言 + 扩展语言」结构、且需要 pcn 的文档体系，
都可通过下面的自动发现契约接入。
...
> **扫描路径取决于技能装在哪**：仓库内为 `skills/translate-*/`；已安装到助手目录
> 时同级为 `<助手目录>/skills/translate-*/`（如 `~/.opencode/skills/`）。**当前 cwd
> 不是技能源仓库时，前者扫不到任何语言**——发现结果为空应退回读取仓库现有语言
> 清单，而不是判定"没有扩展语言"。
...
```bash
# 检查切换器是否包含全部已注册语言
# 语言集动态发现（勿写死数量）：基准语言 + skills/translate-*/ 的 display_name
EXTRA=$(for s in skills/translate-*/SKILL.md; do
          [ -f "$s" ] && sed -n '/^display_name:/s/.*: *//p' "$s"
        done)
NAMES="中文 English 日本語 $EXTRA"          # 换成该仓库的基准语言显示名
EXPECTED=$(printf '%s\n' $NAMES | grep -c .)

for f in $(grep -rl --include='*.md' -E "^(\[?(中文|English|日本語)\]?) \|" docs 2>/dev/null); do
  line=$(grep -m1 -E "^(\[?(中文|English|日本語)\]?) \|" "$f")
  s=0
...
> ⚠️ **三个易错点**（实测踩到）：① `grep '^\[中文\]'` 在 BRE 里把 `[中文]` 当
> **字符类**（匹配单个「中」或「文」），要字面量须 `grep -F`；② **必须用"行首是
> 已知语言名"锚定**，否则会命中 shell 管道与表格行（`grep -m1` 取到的还是出错
> 那一行）；③ **基语言自身在切换器里是纯文本**（无方括号），匹配须宽松。
>
> **更可靠**：若仓库已有文档检查脚本，直接复用并把它语言集改为动态发现。

> ⚠️ **扫描路径取决于技能装在哪**：仓库内是 `skills/translate-*/`；技能已安装到
> 助手目录时，同一级为 `<助手目录>/skills/translate-*/`（如 `~/.opencode/skills/`）。
> **cwd 不是技能源仓库时，前者会静默扫不到任何语言**——发现结果为空就退回读取
> 仓库现有的语言清单，不要当成"没有扩展语言"。
### 6. 本地化文档内链必须指向同语言版本
...
实测：对 55 字候选表做全量扫描，真正的简体专属字**仅 5 个**（`档` `径` `译` `说明` `实例`），其余多为一字多用的日文汉字。**比例随语料而异，此处的数字只是来源仓库的实测值**——要点是"比例低到不能凭字形猜测"这个结论，而非具体计数。
...
依据：该仓库既有的本地化版本（如 `docs/MAINTENANCE.ja.md`）的同一列同样保留中文原样——git 提交信息是**不可变的外部引用**，与仓库自身规范文件（如 `AGENTS.md`）的章节标题同理。
